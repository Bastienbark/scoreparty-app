...
import { DartThrow, Player, ShanghaiLiveGame } from '../../types/models';
...
export function DartsShanghaiLiveScreen() {
  const navigation = useNavigation<HomeStackNavProp<'Live'>>();
  const liveGame = useAppStore((s) => s.liveGame) as ShanghaiLiveGame;
  const players = useAppStore((s) => s.players);
  const dartsShanghaiAddThrow = useAppStore((s) => s.dartsShanghaiAddThrow);
  const dartsShanghaiUndoThrow = useAppStore((s) => s.dartsShanghaiUndoThrow);
  ...
  const game = getGameOrThrow('darts-shanghai');
  const ranking = game.liveRanking(liveGame, playersMap);
  const { scores, winnerIds, shanghaiWinnerId, activePlayerId, round } = deriveShanghaiState(liveGame);
  ...
  const winnerNames = (winnerIds ?? []).map((id) => playersMap[id]?.name);
    ...
    <LiveScreenLayout
      header={
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>Shanghai</Text>
            <Text style={styles.subtitle}>{liveGame.totalRounds} rounds</Text>
          </View>
        </View>
      }
      footer={<Button label={winnerIds ? 'Voir le récap 🎯' : 'Partie en cours…'} disabled={!winnerIds} onPress={finishNow} size="md" />}
    >
      {activePlayer && !winnerIds && (
        <View style={styles.activeCard}>
          ...
          <Text style={styles.targetLabel}>
            Round {round}/{liveGame.totalRounds} · vise
          </Text>
          <Text style={styles.target}>{round}</Text>
      ...
      {winnerIds && (
        <View style={styles.winnerCard}>
          <Text style={styles.winnerText}>
            🎯 {shanghaiWinnerId ? `Shanghai ! ${winnerNames[0]} gagne immédiatement` : `${winnerNames.join(' & ')} gagne${winnerNames.length > 1 ? 'nt' : ''} avec le meilleur score`}
          </Text>
        </View>
      )}
      ...
      <View style={{ gap: 6, marginBottom: 20 }}>
        {ranking.map((r, idx) => (
          <RankingRow
            ...
            highlight={(winnerIds ?? []).includes(r.id) || (idx === 0 && !winnerIds)}
          />
        ))}
      </View>
          ...
          <View style={{ gap: 6 }}>
            {recentTurns.map((t, idx) => (
              <View key={idx} style={styles.turnRow}>
                ...
                <Text style={styles.turnRound}>R{t.round}</Text>
...
const styles = StyleSheet.create({
  ...
  target: { fontFamily: fonts.headingBold, fontSize: 48, fontWeight: '700', color: colors.teal, marginVertical: 2 },
  ...
  turnRound: { fontSize: 11, color: colors.textMuted, width: 24 },
