...
import { CinqRoisLiveGame, Player } from '../../types/models';
import { colors, fonts, radii } from '../../theme/tokens';

const LABEL_COL = 34;
const CELL_COL = 56;

export function CinqRoisLiveScreen() {
  const navigation = useNavigation<HomeStackNavProp<'Live'>>();
  const liveGame = useAppStore((s) => s.liveGame) as CinqRoisLiveGame;
  ...
  const crNextRound = useAppStore((s) => s.crNextRound);
  ...
  const game = getGameOrThrow('cinq-rois');
  const roundNum = liveGame.currentRound;
  const atout = atoutForRound(roundNum);
  const dealerName = playerById(dealerForRound(roundNum, liveGame.playerIds)).name;
